...
export function ThreeCanvas({
  ...
  useEffect(() => {
    ...
    let handle: ThreeRuntimeHandle | null = null;
    const ro = new ResizeObserver(() => {
      sizeCanvas();
      if (canvas && handle) handle.setSize(canvas.width, canvas.height);
    });
    ...
    handle = createThreeRuntime({
      canvas,
      sceneBody,
      duration,
      preserveDrawingBuffer,
      onError: (log) => setError(log),
    });
